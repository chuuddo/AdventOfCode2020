const game = ([deck1, deck2], recursive) => {
  ...
  while (deck1.length > 0 && deck2.length > 0) {
    ...
    if (recursive && deck1.length >= card1 && deck2.length >= card2) {
      p1win = game([deck1.slice(0, card1), deck2.slice(0, card2)], recursive) === 1;
    }
    if (p1win) {
...
const getWinnerScore = (data, recursive) => {
  const decks = data.split("\n\n").map((x) => x.split("\n").slice(1).map(Number));
  const winner = game(decks, recursive);
  return decks[winner - 1].reverse().reduce((acc, x, i) => acc + x * (i + 1), 0);
};

module.exports = {
  part1: (data) => getWinnerScore(data, false),
  part2: (data) => getWinnerScore(data, true),
};
